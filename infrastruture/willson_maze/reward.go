...
func (m *WillsonMaze) PopulateReward(r struct {
	...
	stack := []i.CellPosition{&CellPosition{row: 0, col: 0}}
	startPosKey := "0,0"
...
func pop(s *[]i.CellPosition) i.CellPosition {
	lastIndex := len(*s) - 1
	popped := (*s)[lastIndex]
...
func calcProb(baseProb float32, cell i.CellPosition, mazeWidth, mazeHeight int) float32 {
