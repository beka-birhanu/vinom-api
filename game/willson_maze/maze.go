...
type WillsonMaze struct {
	...
	grid       [][]game.Cell // The 2D grid of cells that form the maze.
...
func (m *WillsonMaze) NewValidMove(curPos game.CellPosition, dir string) (game.Move, error) {
...
func (m *WillsonMaze) RetriveGrid() [][]game.Cell {
	return m.grid
}

// SetGrid sets the 2D grid of cells for the maze (implements game.Maze).
func (m *WillsonMaze) SetGrid(g [][]game.Cell) {
	m.grid = g
}
...
func New(width, height int) (*WillsonMaze, error) {
	...
	grid := make([][]game.Cell, height)
	for i := range grid {
		grid[i] = make([]game.Cell, width)
		for j := range grid[i] {
			grid[i][j] = &Cell{
				northWall: true,
				southWall: true,
...
func (m *WillsonMaze) randomCellPosition() game.CellPosition {
...
func (m *WillsonMaze) randomUnvisitedCellPosition(visited map[string]struct{}) game.CellPosition {
	for {
...
func (m *WillsonMaze) neighbors(pos game.CellPosition) []Move {
	var result []Move
...
func (m *WillsonMaze) getDirection(pos1, pos2 game.CellPosition) string {
...
func (m *WillsonMaze) randomWalk(visited map[string]struct{}) map[game.CellPosition]Move {
	start := m.randomUnvisitedCellPosition(visited)
	visits := make(map[game.CellPosition]Move)
	cell := start
...
func (m *WillsonMaze) Move(move game.Move) (int32, error) {
	if !m.IsValidMove(move) {
...
func (m *WillsonMaze) RemoveReward(pos game.CellPosition) error {
