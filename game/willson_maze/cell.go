...
var _ game.CellPosition = &CellPosition{}
var _ game.Move = &Move{}
var _ game.Maze = &WillsonMaze{}
...
type Move struct {
	from game.CellPosition // The starting position of the move.
	to   game.CellPosition // The destination position of the move.
}

// From returns the starting position of the move.
func (m *Move) From() game.CellPosition {
	return m.from
}

// To returns the destination position of the move.
func (m *Move) To() game.CellPosition {
	return m.to
}

// SetFrom updates the starting position of the move.
func (m *Move) SetFrom(from game.CellPosition) {
	m.from = from
}

// SetTo updates the destination position of the move.
func (m *Move) SetTo(to game.CellPosition) {
	m.to = to
}
